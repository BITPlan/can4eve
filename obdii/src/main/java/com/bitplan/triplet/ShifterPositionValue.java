...
/**
 * Shifter Position of a Triplet Vehicle
 * @author wf
 *
 */
public class ShifterPositionValue extends CANValue<ShifterPosition> {
  ...
  /**
   * construct a new ShifterPosition Value from the given canInfoooo
   * @param canInfo
   */
  public ShifterPositionValue(CANInfo canInfo) {
    super(canInfo, ShifterPosition.class);
    getValueItem().setValue(new ShifterPosition(0x50));
  }
  ...
  public String asString() {
    ...
    if (getValueItem().isAvailable())
      result = String.format("%s", getValueItem().getValue().shiftPosition.toString());
    return result;
  }
}