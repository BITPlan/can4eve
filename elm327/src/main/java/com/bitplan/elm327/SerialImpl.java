...
public class SerialImpl extends ConnectionImpl {
  ...
  public void connect(String device, int baud) {
    ...
    try {
      ...
      serialPort = (SerialPort) serialId.open(this.getClass().getSimpleName(),
          (int) this.timeOut);
    ...
    try {
      ...
      throw new RuntimeException(
          "can not set serial params for device " + device);
    }
    try {
  ...
  public void halt() {
    super.halt();
    close();
    // I am a thread so I need to remove myself for restart
    instance = null;
  }
